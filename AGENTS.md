...
CI는 `--cov-fail-under=90`으로 **하강만** 막는다(측정 기준선 90.30%). 이건
목표치가 아니라 래칫이다 — 커버리지를 목표로 삼으면 의미 없는 테스트를 쓰게
되므로, 덮여 있던 코드가 벗겨지는 경우만 잡는다. 기본 옵션에 넣지 않은 이유는
파일 하나만 돌릴 때도 게이트가 걸려 개발 중에 계속 실패하기 때문이다.
