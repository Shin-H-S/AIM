...
CI는 `--cov-fail-under=89`로 **하강만** 막는다(실측 90.2%). 이건 목표치가 아니라
래칫이다 — 커버리지를 목표로 삼으면 라인만 훑는 무의미한 테스트를 쓰게 되므로,
덮여 있던 코드가 벗겨지는 경우만 잡는다. 실측보다 1pp 낮게 잡은 이유는 기능이
붙을 때마다 전체 비율이 소수점 단위로 흔들리기 때문이다 — 여유가 없으면 정상적인
기능 추가마다 CI가 터진다. 기본 옵션에 넣지 않은 이유는 파일 하나만 돌릴 때도
게이트가 걸려 개발 중에 계속 실패하기 때문이다.
