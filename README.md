...
> 🚧 **개발 중 — 조사 에이전트**: 인시던트가 나면 도구 루프로 검사 결과·시나리오·아티팩트를
> 직접 조사해 원인을 7유형으로 분류하고 조치를 제안하는 LLM 에이전트를 만들고 있습니다.
> 평가부터 먼저 — 실측 사고를 포함한 [평가셋 175건과 채점기](apps/worker/src/aim_worker/agent/)를
> 완성했고, 규칙 베이스라인(정확도 91~93%)을 에이전트가 넘어야 할 목표선으로 고정했습니다.
