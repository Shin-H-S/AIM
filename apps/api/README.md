...
웹에서는 `/projects/{projectId}/scenarios`에서 scenario 목록을 조회하고 active scenario의 수동 ScenarioRun을 생성할 수 있습니다.
...
AIReport 저장을 위한 `ai_reports` 테이블, SQLAlchemy 모델, `aim_api.services.ai_reports.generate_and_record_ai_report` 저장 서비스, CheckRun별 전체 조회 API, CheckRun 상세 응답의 요약 필드도 제공합니다. `ai_reports`는 CheckRun당 하나의 report를 저장하며, 조회와 필터링에 필요한 score/risk 요약 컬럼과 전체 report JSON payload를 함께 보존합니다. Worker는 AIReport 생성 실패를 CheckRun 실패로 바꾸지 않고 로그만 남기므로, AIReport 재시도는 후속 작업에서 분리합니다.
