...
다음 경로에서 프로젝트의 Incident와 Alert 이력을 확인할 수 있습니다.

```text
/projects/{projectId}/alerts
```

페이지는 로그인 페이지에서 저장한 access token을 자동으로 사용하고, 필요한 경우 access token을 직접 입력해 다시 조회할 수 있습니다. 현재 Project의 response time threshold, quality score threshold, Incident 목록, evidence JSON, Alert 발송 상태(채널 배지 포함)와 관련 CheckRun 링크를 표시합니다. Alert 목록은 로드된 항목 기준으로 전체, 대기, 발송됨, 실패 상태 필터를 제공하고, 더 보기 버튼으로 다음 page를 이어서 불러올 수 있습니다.

Alert 채널 설정 폼에서 email alert 사용 여부·수신자 email과 Slack/Discord webhook URL을 저장할 수 있습니다. 수신자 email을 비워두면 Project owner email을 사용하고, webhook URL을 비워두면 webhook 알림을 보내지 않습니다. FAILED 상태의 Alert는 채널과 무관하게 카드에서 발송 재시도를 요청할 수 있으며, 성공하면 상태가 PENDING으로 갱신됩니다.
...
- ScenarioRun 결과 페이지
- Alert overview 페이지
- Alert overview의 alert 채널 설정 저장 폼 (email 사용 여부·수신자, Slack/Discord webhook URL)
- Alert overview의 FAILED alert 발송 재시도 버튼
- Alert overview의 alert 상태 필터와 더 보기 버튼
