...
현재 구현된 worker task는 CheckRun을 `RUNNING`으로 전환하고 HTTP availability scanner, SSL inspection, Lighthouse mobile scan을 실행합니다. API에는 Playwright 시나리오 정의와 ScenarioRun/StepResult 저장 기반이 추가되어 있습니다. worker는 ScenarioRun task를 인식하고 실패 상태를 기록하는 안전장치를 포함하지만, 실제 브라우저 action executor는 아직 포함하지 않았습니다.
