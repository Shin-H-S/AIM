...
- `score_result`: 현재 구현된 scanner 결과 기준의 결정론적 score와 deployment risk입니다. 아직 계산 전이면 `null`입니다.
...
critical step이 실패하면 해당 step은 `FAILED`, 이후 step은 `SKIPPED`로 기록하고 scenario run은 `FAILED`가 됩니다. non-critical step 실패는 step result에는 `FAILED`로 남기지만 전체 scenario run은 계속 진행합니다.

브라우저의 `console.error`는 `console_errors`에 저장됩니다. failed network request는 `network_failures`에 저장됩니다. evidence message와 URL에 포함될 수 있는 token/password/API key 형태의 값은 저장 전에 마스킹하며, SSRF-safe 검증을 통과하지 못한 URL은 `[blocked-url]`로 저장합니다.
