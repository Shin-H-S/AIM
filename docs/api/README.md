...
프로젝트에 Playwright 테스트 시나리오를 생성합니다.
...
현재 API는 step 정의 저장만 지원합니다. 실제 Playwright 실행과 scenario run 결과 저장은 아직 포함하지 않았습니다.

### `GET /projects/{project_id}/scenarios`

프로젝트의 시나리오 목록을 조회합니다.
...
시나리오 단건과 step 목록을 조회합니다.

### `PATCH /projects/{project_id}/scenarios/{scenario_id}`

시나리오 이름, 설명, 활성 여부, step 목록을 수정합니다. `steps`를 보내면 기존 step 목록을 새 목록으로 교체합니다.

### `DELETE /projects/{project_id}/scenarios/{scenario_id}`

시나리오를 삭제합니다. 연결된 step도 함께 삭제됩니다.
...
- Playwright scenario 실행과 scenario run 결과 저장
