...
- `score_result`: scanner 결과와 같은 프로젝트의 최신 terminal ScenarioRun 결과 기준의 결정론적 score와 deployment risk입니다. 아직 계산 전이면 `null`입니다. ScenarioRun이 아직 없으면 `functional_stability_score`는 `null`입니다.
...
critical step이 실패하면 해당 step은 `FAILED`, 이후 step은 `SKIPPED`로 기록하고 scenario run은 `FAILED`가 됩니다. non-critical step 실패는 step result에는 `FAILED`로 남기지만 전체 scenario run은 계속 진행합니다. 실패한 step은 screenshot artifact가 저장된 경우 `failure_screenshot_artifact_id`를 포함합니다.
